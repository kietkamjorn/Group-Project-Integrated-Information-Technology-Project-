describe(`TC-FE-PBI22-VALIDATE-MATCH-PASSWORD-1\n 
    Test Scenario : failed - standard email format\n
                           - max-length+1 email and password`, () => {
    ...
    it('should be enable the Sign In button after type max-length email and max-length password.',()=>{
        ...
        cy.get('@email').type('max-email-validate-match-pssword@ad.sit.kmutt.ac.th')
                        .should('have.value','max-email-validate-match-pssword@ad.sit.kmutt.ac.t')

        cy.get('.itbms-password').as('password')
        cy.get('@password').type('Password2025***')
                           .should('have.value','Password2025**')
